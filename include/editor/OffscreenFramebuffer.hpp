...
public:
  ...
  void resize(uint32_t newWidth, uint32_t newHeight) {
    ...
    if (newWidth == width && newHeight == height && colorImage != nullptr) return;
    ...
    ImageUtils::createImage(
      *pPhysicalDevice, *pLogicalDevice,
      width, height,
      COLOR_FORMAT,
    ...
    colorImageView = ImageUtils::createImageView(*pLogicalDevice, colorImage, COLOR_FORMAT, vk::ImageAspectFlagBits::eColor);
    ...
    depthImageView = ImageUtils::createImageView(*pLogicalDevice, depthImage, depthFormat, vk::ImageAspectFlagBits::eDepth);
  ...
  const vk::raii::Image& getColorImage() const { return colorImage; }
