...
namespace sauce {
    ...
    const std::vector<uint16_t> indices{
        1,  0,  2,  2,  0,  3,  4,  5,  6,  6,  7,  4,  8,  9,  10, 10, 11, 8,
        13, 12, 14, 15, 14, 12, 16, 17, 18, 18, 19, 16, 21, 20, 22, 23, 22, 20,
    };
      ...
      public:
        ...
        Renderer(const RendererCreateInfo& createInfo)
            : pPhysicalDevice(&createInfo.physicalDevice),
              pLogicalDevice(&createInfo.logicalDevice), pRenderSurface(&createInfo.renderSurface),
            ...
            pSwapChain = std::make_unique<sauce::SwapChain>(
                createInfo.physicalDevice, createInfo.logicalDevice, createInfo.renderSurface,
                createInfo.window, createInfo.vsync);
            ...
            sauce::GraphicsPipelineConfig mainPipelineConfig{
                ...
                .descriptorSetLayouts = {*descriptorSetLayout0, *descriptorSetLayout1,
                                         *modeling::Material::getDescriptorSetLayout()},
            ...
            pPostProcessPipeline =
                std::make_unique<sauce::GraphicsPipeline>(postProcessPipelineConfig);
        ...
        const vk::raii::DescriptorSetLayout& getDescriptorSetLayout0() const {
            return descriptorSetLayout0;
        }
        const vk::raii::DescriptorSetLayout& getDescriptorSetLayout1() const {
            return descriptorSetLayout1;
        }
        uint32_t getFrameIndex() const {
            return frameIndex;
        }
        const vk::raii::DescriptorSet& getCurrentDescriptorSet() const {
            return descriptorSets[frameIndex];
        }
        const vk::raii::DescriptorSet& getEnvironmentDescriptorSet() const {
            assert(!environmentDescriptorSets.empty() &&
                   "Environment descriptor sets not initialized!");
            return environmentDescriptorSets[0];
        }
        const vk::raii::DescriptorSet& getDefaultMaterialDescriptorSet() const {
            assert(!defaultMaterialDescriptorSets.empty() &&
                   "Default material descriptor sets not initialized!");
            return defaultMaterialDescriptorSets[0];
        }
        void* getCurrentUniformBufferMapped() const {
        ...
        const vk::raii::Buffer& getCurrentUniformBuffer() const {
            return uniformBuffers[frameIndex];
        }
        const vk::raii::DescriptorPool& getDescriptorPool() const {
            return descriptorPool;
        }
        const vk::raii::ImageView& getDefaultImageView() const {
            return defaultImageView;
        }
        const vk::raii::Sampler& getDefaultSampler() const {
        ...
        void recreateSwapChain() {
            ...
            pSwapChain = std::make_unique<sauce::SwapChain>(*pPhysicalDevice, *pLogicalDevice,
                                                            *pRenderSurface, pWindow);
        ...
        void createDescriptorSetLayout(const sauce::LogicalDevice& logicalDevice) {
            ...
            // Camera UBO
            perFrameBindings[0] = {.binding = 0,
                                   .descriptorType = vk::DescriptorType::eUniformBuffer,
                                   .descriptorCount = 1,
                                   .stageFlags = vk::ShaderStageFlagBits::eVertex |
                                                 vk::ShaderStageFlagBits::eFragment};
            // Light SSBO
            perFrameBindings[1] = {.binding = 1,
            ...
            descriptorSetLayout0 =
                vk::raii::DescriptorSetLayout{*logicalDevice, perFrameDsLayoutInfo};
            ...
            environmentBindings[0] = {.binding = 0,
                                      .descriptorType = vk::DescriptorType::eCombinedImageSampler,
                                      .descriptorCount = 1,
                                      .stageFlags = vk::ShaderStageFlagBits::eFragment};
            environmentBindings[1] = {.binding = 1,
                                      .descriptorType = vk::DescriptorType::eCombinedImageSampler,
                                      .descriptorCount = 1,
                                      .stageFlags = vk::ShaderStageFlagBits::eFragment};
            environmentBindings[2] = {.binding = 2,
                                      .descriptorType = vk::DescriptorType::eCombinedImageSampler,
                                      .descriptorCount = 1,
                                      .stageFlags = vk::ShaderStageFlagBits::eFragment};
            ...
            descriptorSetLayout1 =
                vk::raii::DescriptorSetLayout{*logicalDevice, environmentDsLayoutInfo};
            ...
            postProcessDescriptorSetLayout =
                vk::raii::DescriptorSetLayout{*logicalDevice, ppDsLayoutInfo};
        }
        ...
        void createDescriptorSets(const sauce::LogicalDevice& logicalDevice) {
            ...
            // Environment set
            vk::DescriptorSetAllocateInfo envAllocInfo{.descriptorPool = descriptorPool,
            ...
            vk::DescriptorSetAllocateInfo matAllocInfo{.descriptorPool = descriptorPool,
                                                       .descriptorSetCount = 1,
                                                       .pSetLayouts = &materialLayout};
            defaultMaterialDescriptorSets = logicalDevice->allocateDescriptorSets(matAllocInfo);

            vk::DescriptorBufferInfo lightSSBOInfo{
                .buffer = *lightSSBO, .offset = 0, .range = lightSSBOSize};

            // Fallback for IBL maps
            vk::DescriptorImageInfo iblInfo{.sampler = *defaultSampler,
            ...
            for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; ++i) {
                ...
                writes[1] = {.dstSet = descriptorSets[i],
                             ...
                             .descriptorCount = 1,
                             .descriptorType = vk::DescriptorType::eStorageBuffer,
                             .pBufferInfo = &lightSSBOInfo};

                logicalDevice->updateDescriptorSets(writes, {});
            }

            std::array<vk::WriteDescriptorSet, 3> envWrites;
            envWrites[0] = {.dstSet = environmentDescriptorSets[0],
                            .dstBinding = 0,
                            .descriptorCount = 1,
                            .descriptorType = vk::DescriptorType::eCombinedImageSampler,
                            .pImageInfo = &iblInfo};
            envWrites[1] = {.dstSet = environmentDescriptorSets[0],
                            .dstBinding = 1,
                            .descriptorCount = 1,
                            .descriptorType = vk::DescriptorType::eCombinedImageSampler,
                            .pImageInfo = &iblInfo};
            envWrites[2] = {.dstSet = environmentDescriptorSets[0],
                            .dstBinding = 2,
                            .descriptorCount = 1,
                            .descriptorType = vk::DescriptorType::eCombinedImageSampler,
                            .pImageInfo = &iblInfo};
            ...
            vk::DescriptorBufferInfo matUboInfo{
                .buffer = *materialBuffer, .offset = 0, .range = sizeof(MaterialData)};
            vk::DescriptorImageInfo defaultImageInfo{.sampler = *defaultSampler,
                                                     .imageView = *defaultImageView,
                                                     .imageLayout =
            ...
            for (uint32_t i = 0; i < 5; ++i) {
                matWrites[i] = {.dstSet = defaultMaterialDescriptorSets[0],
                                .dstBinding = i,
                                .descriptorCount = 1,
                                .descriptorType = vk::DescriptorType::eCombinedImageSampler,
                                .pImageInfo = &defaultImageInfo};
            }
            matWrites[5] = {.dstSet = defaultMaterialDescriptorSets[0],
                            .dstBinding = 5,
                            .descriptorCount = 1,
                            .descriptorType = vk::DescriptorType::eUniformBuffer,
        ...
        void createDepthResources(const sauce::PhysicalDevice& physicalDevice,
                                  const sauce::LogicalDevice& logicalDevice) {
            vk::Format depthFormat = GraphicsPipeline::findDepthFormat(physicalDevice);
            ImageUtils::createImage(
                physicalDevice, logicalDevice, pSwapChain->getExtent().width,
                pSwapChain->getExtent().height, depthFormat, vk::ImageTiling::eOptimal,
                vk::ImageUsageFlagBits::eDepthStencilAttachment,
                vk::MemoryPropertyFlagBits::eDeviceLocal, depthImage, depthImageMemory);
            depthImageView = ImageUtils::createImageView(logicalDevice, depthImage, depthFormat,
                                                         vk::ImageAspectFlagBits::eDepth);
        }

        void createOffscreenResources(const sauce::PhysicalDevice& physicalDevice,
                                      const sauce::LogicalDevice& logicalDevice) {
            vk::Format colorFormat = pSwapChain->getSurfaceFormat().format;
            ImageUtils::createImage(
                physicalDevice, logicalDevice, pSwapChain->getExtent().width,
                pSwapChain->getExtent().height, colorFormat, vk::ImageTiling::eOptimal,
                vk::ImageUsageFlagBits::eColorAttachment | vk::ImageUsageFlagBits::eSampled,
                vk::MemoryPropertyFlagBits::eDeviceLocal, offscreenImage, offscreenImageMemory);
            offscreenImageView = ImageUtils::createImageView(
                logicalDevice, offscreenImage, colorFormat, vk::ImageAspectFlagBits::eColor);
        ...
        void createVertexBuffer(const sauce::PhysicalDevice& physicalDevice,
                                const sauce::LogicalDevice& logicalDevice) {
            vk::DeviceSize bufferSize = sizeof(Vertex) * vertices.size();

            vk::raii::Buffer stagingBuffer = nullptr;
            vk::raii::DeviceMemory stagingBufferMemory = nullptr;
            sauce::BufferUtils::createBuffer(physicalDevice, logicalDevice, bufferSize,
            ...
            sauce::BufferUtils::copyBuffer(logicalDevice, commandPool, *pQueue, stagingBuffer,
                                           vertexBuffer, bufferSize);
        }

        void createIndexBuffer(const sauce::PhysicalDevice& physicalDevice,
                               const sauce::LogicalDevice& logicalDevice) {
        ...
        void transitionImageLayout(const vk::raii::CommandBuffer& cmdBuf, vk::Image image,
        ...
        void recordCommandBuffer(uint32_t imageIndex, sauce::ImGuiRenderer* imguiRenderer) {
            ...
            transitionImageLayout(commandBuffers[frameIndex], *offscreenImage,
            ...
            transitionImageLayout(commandBuffers[frameIndex], *depthImage,
                                  vk::ImageLayout::eUndefined,
            ...
            commandBuffers[frameIndex].bindDescriptorSets(vk::PipelineBindPoint::eGraphics,
                                                          pPipeline->getLayout(), 1,
                                                          *environmentDescriptorSets[0], nullptr);
            commandBuffers[frameIndex].bindDescriptorSets(
                vk::PipelineBindPoint::eGraphics, pPipeline->getLayout(), 2,
                *defaultMaterialDescriptorSets[0], nullptr);
            ...
            commandBuffers[frameIndex].pushConstants<uint32_t>(
                *pPipeline->getLayout(), vk::ShaderStageFlagBits::eFragment, 0u, {lightCount});

            commandBuffers[frameIndex].drawIndexed(static_cast<uint32_t>(indices.size()), 1, 0, 0,
                                                   0);

            commandBuffers[frameIndex].endRendering();

            transitionImageLayout(
                commandBuffers[frameIndex], *offscreenImage,
            ...
            transitionImageLayout(commandBuffers[frameIndex], pSwapChain->getImages()[imageIndex],
                                  vk::ImageLayout::eUndefined,
                                  vk::ImageLayout::eColorAttachmentOptimal, {},
                                  vk::AccessFlagBits2::eColorAttachmentWrite,
                                  vk::PipelineStageFlagBits2::eColorAttachmentOutput,
            ...
            commandBuffers[frameIndex].bindPipeline(vk::PipelineBindPoint::eGraphics,
                                                    **pPostProcessPipeline);
            commandBuffers[frameIndex].bindDescriptorSets(vk::PipelineBindPoint::eGraphics,
                                                          pPostProcessPipeline->getLayout(), 0,
                                                          *postProcessDescriptorSets[0], nullptr);
            ...
            transitionImageLayout(
                commandBuffers[frameIndex], pSwapChain->getImages()[imageIndex],
        ...
        void drawFrame(const sauce::LogicalDevice& logicalDevice, const sauce::Scene& scene,
                       sauce::ImGuiRenderer* imguiRenderer = nullptr) {
            ...
            try {
                auto [result, idx] =
                    (*pSwapChain)
                        ->acquireNextImage(UINT64_MAX, *presentCompleteSemaphores[frameIndex],
                                           nullptr);
        ...
        void createDefaultTextures(const sauce::PhysicalDevice& physicalDevice,
                                   const sauce::LogicalDevice& logicalDevice) {
            ...
            sauce::BufferUtils::createBuffer(physicalDevice, logicalDevice, pixelSize,
                                             vk::BufferUsageFlagBits::eTransferSrc,
                                             vk::MemoryPropertyFlagBits::eHostVisible |
                                                 vk::MemoryPropertyFlagBits::eHostCoherent,
            ...
            ImageUtils::createImage(
                physicalDevice, logicalDevice, 1, 1, vk::Format::eR8G8B8A8Unorm,
                vk::ImageTiling::eOptimal,
                vk::ImageUsageFlagBits::eTransferDst | vk::ImageUsageFlagBits::eSampled,
                vk::MemoryPropertyFlagBits::eDeviceLocal, defaultImage, defaultImageMemory);

            ImageUtils::transitionImageLayout(
                logicalDevice, commandPool, *pQueue, defaultImage, vk::ImageLayout::eUndefined,
            ...
            ImageUtils::copyBufferToImage(logicalDevice, commandPool, *pQueue, stagingBuffer,
                                          defaultImage, 1, 1);
            ImageUtils::transitionImageLayout(
            ...
            defaultImageView =
                ImageUtils::createImageView(logicalDevice, defaultImage, vk::Format::eR8G8B8A8Unorm,
                                            vk::ImageAspectFlagBits::eColor);
        ...
        void createMaterialBuffer(const sauce::PhysicalDevice& physicalDevice,
                                  const sauce::LogicalDevice& logicalDevice) {
            MaterialData defaults{};
            sauce::BufferUtils::createBuffer(physicalDevice, logicalDevice, sizeof(MaterialData),
                                             vk::BufferUsageFlagBits::eUniformBuffer,
                                             vk::MemoryPropertyFlagBits::eHostVisible |
                                                 vk::MemoryPropertyFlagBits::eHostCoherent,
        ...
        void createLightSSBO(const sauce::PhysicalDevice& physicalDevice,
                             const sauce::LogicalDevice& logicalDevice) {
            lightSSBOSize = static_cast<vk::DeviceSize>(MAX_LIGHTS) * sizeof(GPULight);
            sauce::BufferUtils::createBuffer(physicalDevice, logicalDevice, lightSSBOSize,
        ...
        const vk::raii::Buffer& getMaterialBuffer() const {
