...
namespace sauce {
    ...
    struct LogicalDevice {
        ...
        LogicalDevice(const sauce::PhysicalDevice& physicalDevice,
                      const sauce::RenderSurface& surface) {
            std::vector<vk::QueueFamilyProperties> queueFamilyProps =
                (*physicalDevice).getQueueFamilyProperties();

            uint32_t graphicsAndPresentIndex;

            auto graphicsQueueFamilyProp =
                std::ranges::find_if(queueFamilyProps, [&](const vk::QueueFamilyProperties& prop) {
            ...
            queueIndex = static_cast<uint32_t>(
                std::distance(queueFamilyProps.begin(), graphicsQueueFamilyProp));
