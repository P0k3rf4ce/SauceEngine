...
public:
    ...
    modeling::Transform& getTransform() { return transform; }
    const modeling::Transform& getTransform() const { return transform; }

    // Convenience accessors
    glm::vec3 getTranslation() const { return transform.getTranslation(); }
    glm::quat getRotation() const { return transform.getRotation(); }
    glm::vec3 getScale() const { return transform.getScale(); }

    void setTranslation(const glm::vec3& translation) { transform.setTranslation(translation); }
    void setRotation(const glm::quat& rotation) { transform.setRotation(rotation); }
