...
namespace sauce {
    ...
    struct PhysicalDevice {
        ...
        PhysicalDevice(const sauce::Instance& instance) {
            ...
            const auto devIter =
                std::ranges::find_if(devices, [&](const vk::raii::PhysicalDevice& device) {
                    bool supportsVulkan1_3 =
                        device.getProperties().apiVersion >= VK_API_VERSION_1_3;
                    std::vector<vk::QueueFamilyProperties> queueFamilies =
                        device.getQueueFamilyProperties();
                    bool supportsGraphicsAndPresent = checkQueueFamilySupport(
                        device.getQueueFamilyProperties(),
                        vk::QueueFlagBits::eGraphics | vk::QueueFlagBits::eGraphics);
                    bool supportsGraphics =
                        std::ranges::any_of(queueFamilies, [](const vk::QueueFamilyProperties& qf) {
                            return !!(qf.queueFlags & vk::QueueFlagBits::eGraphics);
                        });

                    bool supportsAllRequiredDeviceExtensions =
                        checkRequiredExtensions(device, requiredExtensions);
                    bool supportsRequiredFeatures = checkRequiredFeatures(device);

                    return supportsVulkan1_3 && supportsGraphicsAndPresent &&
                           supportsAllRequiredDeviceExtensions && supportsRequiredFeatures;
                });
        ...
        static bool checkRequiredExtensions(const vk::raii::PhysicalDevice& device,
                                            const std::vector<const char*> requiredExtensions) {
            auto availableExtensionProps = device.enumerateDeviceExtensionProperties();
            return std::ranges::all_of(
                requiredExtensions, [&availableExtensionProps](const char* ext) {
                    return std::ranges::any_of(availableExtensionProps,
        ...
        static bool checkRequiredFeatures(const vk::raii::PhysicalDevice& device) {
            ...
            return availableFeatures.get<vk::PhysicalDeviceVulkan11Features>()
                   ...
                   availableFeatures.get<vk::PhysicalDeviceExtendedDynamicStateFeaturesEXT>()
                       .extendedDynamicState;
        }

        static bool checkQueueFamilySupport(const std::vector<vk::QueueFamilyProperties>& qfProps,
                                            vk::QueueFlags mask) {
