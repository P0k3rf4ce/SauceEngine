...
void SettingsManager::load(const std::string& path) {
  ...
  try {
    ...
    if (j.contains("imgui_scale"))       settings.imguiScale       = j["imgui_scale"].get<float>();
    if (j.contains("vsync"))             settings.vsync             = j["vsync"].get<bool>();
    if (j.contains("working_directory")) settings.workingDirectory  = j["working_directory"].get<std::string>();
    if (j.contains("palantir_mode"))     settings.palantirMode      = j["palantir_mode"].get<bool>();
    if (j.contains("show_debug_stats"))  settings.showDebugStats    = j["show_debug_stats"].get<bool>();
    if (j.contains("mouse_sensitivity")) settings.mouseSensitivity  = j["mouse_sensitivity"].get<float>();
    if (j.contains("camera_speed"))      settings.cameraSpeed       = j["camera_speed"].get<float>();
    if (j.contains("field_of_view"))     settings.fieldOfView       = j["field_of_view"].get<float>();
