    ...
    namespace modeling {
        ...
        void Material::initDescriptorSetLayout(const sauce::LogicalDevice& logicalDevice) {
            ...
            descriptorSetLayout = std::make_unique<vk::raii::DescriptorSetLayout>(
                *logicalDevice, materialDsLayoutInfo);
        }
        ...
        void Material::updateUniformBuffer(const sauce::LogicalDevice& logicalDevice) const {
            ...
            ubo.emissiveFactor_alphaCutoff =
                glm::vec4(properties.emissiveFactor, properties.alphaCutoff);
        ...
        void Material::initVulkanResources(const sauce::LogicalDevice& logicalDevice,
            ...
            sauce::BufferUtils::createBuffer(physicalDevice, logicalDevice, sizeof(MaterialUBO),
            ...
            for (uint32_t i = 0; i < 5; ++i) {
                writes.push_back({.dstSet = **descriptorSet,
                                  .dstBinding = i,
                                  .descriptorCount = 1,
                                  .descriptorType = vk::DescriptorType::eCombinedImageSampler,
        ...
        std::vector<vk::DescriptorBufferInfo> Material::getDescriptorBufferInfos() const {
            ...
            if (uniformBuffer) {
                infos.push_back(vk::DescriptorBufferInfo{
                    .buffer = **uniformBuffer, .offset = 0, .range = sizeof(MaterialUBO)});
            }
            return infos;
        }

        std::vector<vk::DescriptorImageInfo> Material::getDescriptorImageInfos(
            const vk::raii::ImageView& defaultView, const vk::raii::Sampler& defaultSampler) const {
            std::vector<vk::DescriptorImageInfo> infos;
            // Order: Albedo, Normal, MetallicRoughness, Emissive, AO
            std::array<TextureType, 5> types = {TextureType::BaseColor, TextureType::Normal,
                                                TextureType::MetallicRoughness,
                                                TextureType::Emissive, TextureType::Occlusion};
