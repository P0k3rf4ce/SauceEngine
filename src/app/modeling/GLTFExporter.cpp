...
GLTFExporter::GLTFExporter(const ExportOptions& options)
    : options(options) {
}
...
bool GLTFExporter::exportScene(const Scene& scene, const std::string& filePath) {
    ...
    if (options.writeBinary) {
        success = writer.WriteGltfSceneToFile(&model, filePath, options.embedImages,
...
int GLTFExporter::getOrCreateMesh(tinygltf::Model& model, const Entity& entity) {
    ...
    if (gltfMesh.primitives.empty()) return -1;
...
int GLTFExporter::getOrCreateMaterial(tinygltf::Model& model, const Material& material) {
    ...
    gltfMat.pbrMetallicRoughness.roughnessFactor = static_cast<double>(props.roughnessFactor);

    // Emissive
    gltfMat.emissiveFactor = {
        static_cast<double>(props.emissiveFactor.r),
    ...
    switch (props.alphaMode) {
        case MaterialProperties::AlphaMode::Opaque: gltfMat.alphaMode = "OPAQUE"; break;
        case MaterialProperties::AlphaMode::Mask:   gltfMat.alphaMode = "MASK"; break;
        case MaterialProperties::AlphaMode::Blend:  gltfMat.alphaMode = "BLEND"; break;
    }
    ...
    setTextureInfo(TextureType::MetallicRoughness, gltfMat.pbrMetallicRoughness.metallicRoughnessTexture);
...
int GLTFExporter::getOrCreateImage(tinygltf::Model& model, const Texture& texture) {
    ...
    if (!texture.isEmbedded() && !texture.getPath().empty()) {
            ...
            if (relPath.string().starts_with("..")) {
                ...
                try {
                    if (texPath != destPath && std::filesystem::exists(texPath)) {
                        std::filesystem::copy_file(texPath, destPath,
        ...
        if (!data.empty()) {
            ...
            if (texture.getChannels() < 4 && gltfImage.image.size() == static_cast<size_t>(texture.getWidth() * texture.getHeight() * texture.getChannels())) {
                // Convert to RGBA
                std::vector<unsigned char> rgba(texture.getWidth() * texture.getHeight() * 4);
...
int GLTFExporter::addAccessor(tinygltf::Model& model,
