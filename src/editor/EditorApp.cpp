...
namespace sauce::editor {
    ...
    EditorApp::EditorApp() {
        ...
        settingsManager.setOnChangeCallback(
            [this](const sauce::EditorSettings& s) { applySettings(s); });
    }
    ...
    void EditorApp::mouseButtonCallback(GLFWwindow* window, int button, int action, int /*mods*/) {
        ...
        if (button == GLFW_MOUSE_BUTTON_LEFT) {
            if (action == GLFW_PRESS) {
                ...
                if (app->viewportHovered && app->selectionManager.hasSelection() &&
                    app->pGizmoRenderer) {
                    ...
                    if (gizmo && entity) {
                        ...
                        if (tc) {
                            ...
                            Ray ray = app->editorCamera.screenToWorldRay(localX, localY, vpSize.x,
                                                                         vpSize.y);

                            float dist =
                                glm::length(app->editorCamera.getPosition() - tc->getTranslation());
        ...
        if (app->gizmoInteracting && app->leftMouseDown && app->pGizmoRenderer) {
            ...
            if (gizmo && entity) {
                ...
                if (tc) {
                    ...
                    Ray ray =
                        app->editorCamera.screenToWorldRay(localX, localY, vpSize.x, vpSize.y);
    ...
    void EditorApp::keyCallback(GLFWwindow* window, int key, int /*scancode*/, int action,
                                int mods) {
        auto* app = static_cast<EditorApp*>(glfwGetWindowUserPointer(window));
        if (!app)
            return;
        ...
        if (action == GLFW_PRESS && ctrl) {
            if (key == GLFW_KEY_S && shift) {
                ...
                std::string defaultPath =
                    app->pScene->hasFilePath()
                        ? app->pScene->getCurrentFilePath()
                        : (std::filesystem::current_path() / "assets" / "scene.gltf").string();
                std::strncpy(app->dialogPathBuf, defaultPath.c_str(),
                             sizeof(app->dialogPathBuf) - 1);
        ...
        if (action == GLFW_PRESS && app->editorCamera.getMode() != EditorCamera::Mode::Fly &&
            ...
            if (key == GLFW_KEY_W) {
                ...
                if (app->pGizmoRenderer)
                    app->pGizmoRenderer->setActiveGizmo(GizmoType::Translate);
                app->setStatusMessage("Gizmo: Translate");
            }
            if (key == GLFW_KEY_E) {
                app->activeGizmoMode = GizmoType::Rotate;
                if (app->pGizmoRenderer)
                    app->pGizmoRenderer->setActiveGizmo(GizmoType::Rotate);
                app->setStatusMessage("Gizmo: Rotate");
            }
            if (key == GLFW_KEY_R) {
                app->activeGizmoMode = GizmoType::Scale;
                if (app->pGizmoRenderer)
                    app->pGizmoRenderer->setActiveGizmo(GizmoType::Scale);
    ...
    void EditorApp::applySettings(const sauce::EditorSettings& s) {
        ...
        if (!s.workingDirectory.empty() && s.workingDirectory != lastWorkingDirectory) {
            ...
            if (ec) {
                SAUCE_LOG("Settings", "Failed to set working directory to '{}': {}",
                          s.workingDirectory, ec.message());
            } else {
                lastWorkingDirectory = s.workingDirectory;
                SAUCE_LOG_VERBOSE("Settings", "Working directory set to '{}'",
