...
GizmoMeshData RotateGizmo::generateMesh() const {
  ...
  auto addRing = [&](GizmoAxis axis) {
    ...
    for (int i = 0; i <= RING_SEGMENTS; ++i) {
      float ringAngle = 2.0f * PI * static_cast<float>(i) / static_cast<float>(RING_SEGMENTS);
      // Center of tube cross-section on the ring
      glm::vec3 ringCenter = (tangent * std::cos(ringAngle) + bitangent * std::sin(ringAngle)) * RING_RADIUS;
      ...
      for (int j = 0; j <= TUBE_SEGMENTS; ++j) {
        float tubeAngle = 2.0f * PI * static_cast<float>(j) / static_cast<float>(TUBE_SEGMENTS);
        glm::vec3 offset = (tubeRight * std::cos(tubeAngle) + tubeUp * std::sin(tubeAngle)) * TUBE_RADIUS;
    ...
    for (int i = 0; i < RING_SEGMENTS; ++i) {
      for (int j = 0; j < TUBE_SEGMENTS; ++j) {
        uint32_t a = base + static_cast<uint32_t>(i) * stride + static_cast<uint32_t>(j);
        uint32_t b = a + stride;
        uint32_t c = a + 1;
        uint32_t d = b + 1;
...
float RotateGizmo::angleOnPlane(const Ray& ray, const glm::vec3& center, const glm::vec3& normal) const {
  float denom = glm::dot(normal, ray.direction);
