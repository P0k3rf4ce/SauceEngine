...
void AnimationProperties::computeCentreOfMassAndVolume(
    ...
    for (const auto& mesh: meshes) {
        ...
        for(auto indices_iter = indices.begin(); indices_iter != indices.end(); indices_iter += 3) {
            ...
            const auto& vertex0 = vertices[indices_iter[0]].Position;
            A.col(0) = Eigen::Vector3d{ vertex0.x, vertex0.y, vertex0.z };
            const auto& vertex1 = vertices[indices_iter[1]].Position;
            A.col(1) = Eigen::Vector3d{ vertex1.x, vertex1.y, vertex1.z };
            const auto& vertex2 = vertices[indices_iter[2]].Position;
            A.col(2) = Eigen::Vector3d{ vertex2.x, vertex2.y, vertex2.z };
    ...
    const Eigen::Vector3d &com) const 
{
    ...
    for (const auto& mesh: meshes) {
        ...
        for (size_t i = 0; i < indices.size(); i += 3) {
            auto& temp_glm_vertex = vertices[indices[i]].Position;
            const Eigen::Vector3d v0 = { temp_glm_vertex.x, temp_glm_vertex.y, temp_glm_vertex.z };
            temp_glm_vertex = vertices[indices[i + 1]].Position;
            const Eigen::Vector3d v1 = { temp_glm_vertex.x, temp_glm_vertex.y, temp_glm_vertex.z };
            temp_glm_vertex = vertices[indices[i + 2]].Position;
            const Eigen::Vector3d v2 = { temp_glm_vertex.x, temp_glm_vertex.y, temp_glm_vertex.z };
